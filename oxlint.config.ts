...
export default defineConfig({
  ...
  ignorePatterns: [
    ...
    // Generated by the Paraglide Vite plugin (like routeTree.gen.ts).
    "src/paraglide",
    "vitest-setup.ts",
  ],
  rules: {
  ...
  overrides: [
    ...
    {
      // Plain Node build/CI scripts legitimately run top-level statements, so the
      // vitest test-hygiene rule (on via the correctness category, and matched by
      // the test override's broad `!e2e/**` glob) doesn't apply. Last override wins.
